...
serve(async (req) => {
  ...
  try {
    ...
    if (!user || !apiKey || !username) {
      return new Response(
        JSON.stringify({ error: 'Authenticated user and user-specific Mspace credentials required' }),
        { 
          status: 401,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    for (let i = 0; i < recipients.length; i += batchSize) {
      ...
      const batchPromises = batch.map(async (recipient) => {
        try {
          ...
          const cost = await storeMessageResult(result, user.id, campaignId, actualSenderId, message, result as unknown as Record<string, unknown>)
